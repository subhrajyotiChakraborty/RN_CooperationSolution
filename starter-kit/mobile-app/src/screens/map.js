  ...
  const onMessage = event => {
    ...
    if (message.status && message.status === 'initialized') {
      Geolocation.getCurrentPosition(position => {
        if (props.route.params && props.route.params.userLocation) {
          const userLocationStr = props.route.params.userLocation;
          const userLocation = userLocationStr.split(',');
          position.coords.latitude = userLocation && userLocation[0];
          position.coords.longitude = userLocation && userLocation[1];
        }
